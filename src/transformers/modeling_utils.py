...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        # hack for now
        filter_m = kwargs.pop("filter_m", None)
        filter_k = kwargs.pop("filter_k", None)
        ...
        if from_tf:
            ...
            for key in state_dict.keys():
                ...
                if filter_k:
                    import re
                    res = re.search('\d+', key)
                    new_key = key.replace("roberta.", "filter.")

                    if res:
                        layer_idx = int(res.group())
                        if layer_idx >= filter_m:
                            if layer_idx < filter_m + filter_k:
                                new_key = new_key.replace("filter.encoder.layer.{}".format(layer_idx), "filter.fusion.{}".format(layer_idx-filter_m))
                            else:
                                # hack for now, "roberta.encoder.layer.3" -> " layer.0"
                                new_key = new_key.replace("filter.encoder.layer.{}".format(layer_idx), "filter.domain.{}".format(layer_idx-filter_m-filter_k))
